// ============================================
  ...
  function addToCart(productId, size, qty = 1) {
    if (qty < 1) qty = 1;
    // Merge: if same productId + size already in cart, bump its qty instead of adding a new line.
    const existing = state.cart.find(i => i.productId === productId && i.size === size);
    if (existing) {
      existing.qty = (existing.qty || 1) + qty;
    } else {
      state.cart.push({ productId, size, qty, addedAt: Date.now() });
    }
    updateCartCount();
    const qtyLabel = qty > 1 ? ` ×${qty}` : '';
    showToast(`Added to cart · ${PRODUCTS[productId].name} (${size})${qtyLabel}`);
  }
  ...
  function setCartLineQty(idx, newQty) {
    ...
    if (newQty > 999) newQty = 999; // sanity cap
    line.qty = newQty;
    updateCartCount();
    renderCart();
  }

  // Sum of qty across all cart lines. Replaces state.cart.length wherever
  // we actually mean "unit count" (tier math, header badge, summaries).
  function cartUnitCount() {
    return state.cart.reduce((sum, item) => sum + (item.qty || 1), 0);
  }
 ...
 // ============================================
  ...
  function currentCartTier() {
    return getWholesaleTier(cartUnitCount());
  }
  ...
  function cartTotal() {
    ...
    if (isWholesale) {
      ...
      return state.cart.reduce((sum, item) => {
        ...
        if (!p) return sum;
        return sum + wholesalePriceAtTier(p.price, tier) * (item.qty || 1);
      }, 0);
    }
    return state.cart.reduce((sum, item) => {
      const p = PRODUCTS[item.productId];
      return p ? sum + p.price * (item.qty || 1) : sum;
    }, 0);
  } 

  function updateCartCount() {
    document.querySelector('.header__cart-count').textContent = `[${cartUnitCount()}]`;
  }
  ...
  function hydrateProduct(id) {
    ...
    Object.entries(p.sizes).forEach(([s, status]) => {
      ...
      if (status !== 'sold') {
        btn.addEventListener('click', () => {
          ...
          const isWholesale = document.body.classList.contains('is-wholesale');
          const unit = isWholesale ? wholesalePriceAtTier(p.price, currentCartTier()) : p.price;
          const qty = parseInt(addBtn.dataset.qty || '1', 10) || 1;
          addBtn.textContent = qty > 1
            ? `Add to cart — $${unit * qty} (${qty} × $${unit})`
            : `Add to cart — $${unit}`;
        });
      }
    ...
    // Inject qty stepper before the add button if not already present.
    // Lives in addBtn.parentNode so it sits inline with the button.
    let qtyEl = document.getElementById('pd-qty');
    if (!qtyEl && addBtn.parentNode) {
      qtyEl = document.createElement('div');
      qtyEl.id = 'pd-qty';
      qtyEl.className = 'pd__qty';
      qtyEl.setAttribute('style', 'display:inline-flex;align-items:center;gap:10px;margin-right:14px;font-family:var(--mono);font-size:13px;letter-spacing:0.1em;vertical-align:middle;');
      qtyEl.innerHTML = `
        <span style="color:var(--ink-soft);text-transform:uppercase;">Qty</span>
        <button type="button" id="pd-qty-dec" aria-label="Decrease quantity" style="width:30px;height:30px;border:1px solid var(--line, #d8d4cb);background:transparent;cursor:pointer;font:inherit;color:inherit;line-height:1;">−</button>
        <span id="pd-qty-val" style="min-width:22px;text-align:center;">1</span>
        <button type="button" id="pd-qty-inc" aria-label="Increase quantity" style="width:30px;height:30px;border:1px solid var(--line, #d8d4cb);background:transparent;cursor:pointer;font:inherit;color:inherit;line-height:1;">+</button>
      `;
      addBtn.parentNode.insertBefore(qtyEl, addBtn);
    }
    // Reset qty UI to 1 on every product hydrate
    const qtyValEl = document.getElementById('pd-qty-val');
    if (qtyValEl) qtyValEl.textContent = '1';

    const getQty = () => parseInt(addBtn.dataset.qty || '1', 10) || 1;
    const setQty = (n) => {
      if (n < 1) n = 1;
      if (n > 999) n = 999;
      addBtn.dataset.qty = String(n);
      if (qtyValEl) qtyValEl.textContent = String(n);
      // Refresh the Add to cart button label to reflect qty × price
      if (addBtn.dataset.size) {
        const isWholesale = document.body.classList.contains('is-wholesale');
        const unit = isWholesale ? wholesalePriceAtTier(p.price, currentCartTier()) : p.price;
        addBtn.textContent = n > 1
          ? `Add to cart — $${unit * n} (${n} × $${unit})`
          : `Add to cart — $${unit}`;
      }
    };
    const decBtn = document.getElementById('pd-qty-dec');
    const incBtn = document.getElementById('pd-qty-inc');
    if (decBtn) decBtn.onclick = () => setQty(getQty() - 1);
    if (incBtn) incBtn.onclick = () => setQty(getQty() + 1);

    addBtn.onclick = () => {
      if (!addBtn.dataset.size) return;
      const qty = getQty();
      addToCart(id, addBtn.dataset.size, qty);
      const isWholesale = document.body.classList.contains('is-wholesale');
      const unit = isWholesale ? wholesalePriceAtTier(p.price, currentCartTier()) : p.price;
      const orig = qty > 1
        ? `Add to cart — $${unit * qty} (${qty} × $${unit})`
  ...
  function renderCart() {
    ...
    document.getElementById('cart-page-count').textContent = `${unitCount} item${unitCount !== 1 ? 's' : ''}`;
    ...
    const linesHTML = state.cart.map((item, idx) => {
      ...
      const qtyControl = `
            <div class="cart-line__qty" style="display:inline-flex;align-items:center;gap:8px;margin-top:8px;font-family:var(--mono);font-size:12px;letter-spacing:0.08em;">
              <button type="button" class="cart-line__qty-btn" data-qty-dec="${idx}" aria-label="Decrease quantity" style="width:24px;height:24px;border:1px solid var(--line, #d8d4cb);background:transparent;cursor:pointer;font:inherit;color:inherit;line-height:1;">−</button>
              <span class="cart-line__qty-val" style="min-width:18px;text-align:center;">${qty}</span>
              <button type="button" class="cart-line__qty-btn" data-qty-inc="${idx}" aria-label="Increase quantity" style="width:24px;height:24px;border:1px solid var(--line, #d8d4cb);background:transparent;cursor:pointer;font:inherit;color:inherit;line-height:1;">+</button>
            </div>`;
      const priceBlock = qty > 1
        ? `<div class="cart-line__price">$${lineTotal}${wholesaleNote}<div style="font-family:var(--mono);font-size:10px;letter-spacing:0.12em;color:var(--ink-soft);margin-top:4px;">$${displayPrice} × ${qty}</div></div>`
      ...
      return `
        <div class="cart-line">
          ...
          <div>
            ...
            ${qtyControl}
            <a class="cart-line__remove" data-remove="${idx}" style="display:block;margin-top:8px;">Remove</a>
          </div>
          ${priceBlock}
        </div>`;
    }).join('');
   ...
   const isWholesaleCart = document.body.classList.contains('is-wholesale');
    ...
    // Wire qty stepper buttons
    grid.querySelectorAll('[data-qty-dec]').forEach(el => {
      el.addEventListener('click', (e) => {
        e.preventDefault();
        const idx = parseInt(el.dataset.qtyDec, 10);
        const line = state.cart[idx];
        if (!line) return;
        setCartLineQty(idx, (line.qty || 1) - 1);
      });
    });
    grid.querySelectorAll('[data-qty-inc]').forEach(el => {
      el.addEventListener('click', (e) => {
        e.preventDefault();
        const idx = parseInt(el.dataset.qtyInc, 10);
        const line = state.cart[idx];
        if (!line) return;
        setCartLineQty(idx, (line.qty || 1) + 1);
      });
  ...
  function populateOrderSummary() {
    ...
    const linesHTML = state.cart.map(item => {
      ...
      const unitPrice = isWholesale ? wholesalePriceAtTier(p.price, summaryTier) : p.price;
      const qty = item.qty || 1;
      ...
      const thumbHTML = photoB64
        ...
        : '';
      const qtyLine = qty > 1 ? ` · Qty ${qty}` : '';
      const priceCell = qty > 1
        ? `<div class="request-order-line__price">$${lineTotal}<div style="font-family:var(--mono);font-size:10px;letter-spacing:0.12em;color:var(--ink-soft);margin-top:2px;">$${unitPrice} × ${qty}</div></div>`
        : `<div class="request-order-line__price">$${unitPrice}</div>`;
      return `
        <div class="request-order-line">
          ${thumbHTML}
          <div class="request-order-line__name">
            ${p.name}
            <small>Size ${item.size}${qtyLine}</small>
          </div>
          ${priceCell}
        </div>`;
    }).join('');
    ...
    const totalUnits = cartUnitCount();
    const wholesaleNote = isWholesale
      ? `<div class="request-order-summary__wholesale-note">★ Wholesale pricing applied (${orderTier.name} · ${Math.round(orderTier.discount*100)}% off · ${totalUnits} units)</div>`
      : '';
    requestOrderSummary.innerHTML = `
      <div class="request-order-summary__title">Your selection (${totalUnits} item${totalUnits !== 1 ? 's' : ''})</div>
      ${linesHTML}
  ...
  requestOrderSubmit?.addEventListener('click', async () => {
    ...
    const itemsList = state.cart.map(item => {
      const p = PRODUCTS[item.productId];
      if (!p) return '';
      const unitPrice = isWholesale ? wholesalePriceAtTier(p.price, submitTier) : p.price;
      const qty = item.qty || 1;
      const lineTotal = unitPrice * qty;
      subtotal += lineTotal;
      return qty > 1
        ? `${p.name} / Size ${item.size} / Qty ${qty} / $${unitPrice} ea / $${lineTotal} line total`
        : `${p.name} / Size ${item.size} / $${unitPrice}`;
    }).join('\n');
    ...
    try {
      await submitToFormspree({
        ...
        is_wholesale: isWholesale ? `YES (${submitTier.name} · ${Math.round(submitTier.discount*100)}% off applied)` : 'No (retail)',
        notes: notes || '(none)',
        items: itemsList,
