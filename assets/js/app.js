// ============================================
  ...
  function hydrateProduct(id) {
    ...
    const photoMap = {
      'hellstar-records-iii': window.__PRODUCT_PHOTOS?.hellstar,
      'gallery-dept-gpatch': window.__PRODUCT_PHOTOS?.galleryDept,
      'vale-forever-classico': window.__PRODUCT_PHOTOS?.valeForever,
      'vale-forever-devils': window.__PRODUCT_PHOTOS?.valeDevils,
      'bape-shark': window.__PRODUCT_PHOTOS?.bape,
      'sp5der-web': window.__PRODUCT_PHOTOS?.sp5der,
      'mmy-hank-low': window.__PRODUCT_PHOTOS?.mmyHank,
      'supreme-satin-applique-hoodie': window.__PRODUCT_PHOTOS?.supremeSatin,
      'bape-broken-college-tee': window.__PRODUCT_PHOTOS?.bapeBroken,
      'bape-sakura-tree-tee': window.__PRODUCT_PHOTOS?.bapeSakura,
    };
  ...
  function renderSearchResults(query) {
    ...
    if (productMatches.length > 0) {
      ...
      html += productMatches.map(([id, p]) => {
        ...
        const photoKey = PHOTO_KEY_BY_PRODUCT_ID[id];
        const photoB64 = photoKey ? window.__PRODUCT_PHOTOS?.[photoKey] : null;
